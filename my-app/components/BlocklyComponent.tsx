...
javascriptGenerator.INFINITE_LOOP_TRAP = 'if (window.loopTrap) window.loopTrap();\n';
...
const BlocklyComponent = () => {
  ...
  useEffect(() => {
    ...
    const onWorkspaceWheel = (e: WheelEvent) => {
      e.preventDefault();
      if (workspace) {
        workspace.scroll(workspace.scrollX + e.deltaX, workspace.scrollY + e.deltaY);
      }
    };
    ...
    if (blocklyDivEl) {
      ...
      // Add wheel listeners
      blocklyDivEl.addEventListener('wheel', onWorkspaceWheel, { passive: false });
      toolboxDiv = (workspace.getToolbox() as any).HtmlDiv;
      if (toolboxDiv) {
        toolboxDiv.addEventListener('wheel', onToolboxWheel, { passive: false });
      }
    }
